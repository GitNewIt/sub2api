...
func convertOpenAIModelListToCodexManifestForAccount(body []byte, account *Account) []byte {
	...
	if _, directMetadata, parseErr := extractUpstreamModelCatalog(body, false); parseErr == nil {
		for modelID, metadata := range directMetadata {
			override := modelMetadata[modelID]
			capabilities := override.CodexToolCapabilities
			override.UpstreamModelMetadata = metadata
			if len(capabilities) > 0 {
				if override.CodexToolCapabilities == nil {
					override.CodexToolCapabilities = make(map[string]json.RawMessage)
				}
				applyCodexToolCapabilities(override.CodexToolCapabilities, capabilities, false)
			}
			modelMetadata[modelID] = override
